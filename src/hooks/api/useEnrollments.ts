...
export const useEnrollment = (courseId: string) => {
  return useQuery({
    queryKey: queryKeys.student.enrollment(courseId),
...
export const useCheckout = () => {
  ...
  return useMutation({
    ...
    onSuccess: () => {
      ...
      queryClient.invalidateQueries({ queryKey: queryKeys.student.enrollments });
    },
...
export const useMarkLessonProgress = (courseId: string) => {
  ...
  return useMutation({
    ...
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: queryKeys.student.enrollment(courseId) });
      queryClient.invalidateQueries({ queryKey: queryKeys.student.enrollments });
    },
    onError: (error: AxiosError<{ message?: string }>) => {
