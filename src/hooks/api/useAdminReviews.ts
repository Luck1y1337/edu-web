...
export const useAdminReviews = (params?: Record<string, unknown>) =>
  useQuery({ queryKey: queryKeys.admin.reviews(params), queryFn: () => adminApi.getReviews(params) });

export const useModerateReview = () => {
  const qc = useQueryClient();
  return useMutation({
    mutationFn: ({ id, body }: { id: string; body: ModerateReviewDto }) => adminApi.moderateReview(id, body),
    onSuccess: () => { toast.success("Sharh moderatsiya qilindi"); qc.invalidateQueries({ queryKey: queryKeys.admin.reviews() }); },
